<div align="center">

# ☕ Bambu NFC

**Чтение и перезапись меток катушек Bambu Lab через PN532**

[![License: MIT](https://img.shields.io/badge/license-MIT-blue)](LICENSE)
![Platform](https://img.shields.io/badge/platform-macOS-lightgrey)
![Python](https://img.shields.io/badge/python-3.8%2B-3776AB?logo=python&logoColor=white)

[![English](https://img.shields.io/badge/lang-English-lightgrey?style=for-the-badge)](README.md)
[![Русский](https://img.shields.io/badge/lang-Русский-2ea44f?style=for-the-badge)](README.ru.md)

[![Buy Me a Coffee](https://img.shields.io/badge/Buy%20Me%20a%20Coffee-support-FFDD00?style=for-the-badge&logo=buymeacoffee&logoColor=black)](https://buymeacoffee.com/disaldr)

</div>

---

Инструмент для работы с RFID-метками катушек филамента **Bambu Lab** (MIFARE Classic 1K)
через ридер **PN532** на macOS. Определяет состояние метки, показывает данные филамента,
снимает дампы и перезаписывает метку вашими данными — **URL, текстом, данными Wi-Fi
или произвольными байтами** (как стандартный NDEF-тег).

Ключи шифрования метки (KeyA и KeyB) **вычисляются из UID** — крэк не требуется:

- `KeyA[sector] = HKDF-SHA256(UID, salt=master, info="RFID-A\0")`
- `KeyB[sector] = HKDF-SHA256(UID, salt=master, info="RFID-B\0")`

> На новых тегах (напр. PLA Lite) KeyB **не нулевой**, вопреки старой документации —
> он тоже выводится из UID, только с контекстом `RFID-B`.

---

## Требования

### Железо
- Модуль **PN532** в режиме **UART**, подключённый через USB-UART переходник
  (обычно чип **CH340**, определяется как `/dev/cu.usbserial-*`).
- Mac (Apple Silicon; инструкция под Homebrew в `/opt/homebrew`).

### ПО
- [Homebrew](https://brew.sh)
- `libnfc` (ридер), компилятор C (Xcode Command Line Tools), Python 3.8+

---

## Установка

### 1. Зависимости

```bash
# Command Line Tools (компилятор cc), если ещё нет
xcode-select --install

# libnfc
brew install libnfc
```

Python-скрипт использует только стандартную библиотеку — доп. пакеты не нужны.

### 2. Подключите ридер

Это всё — `bambu_nfc.py` находит порт сам при каждом запуске. Он перебирает
USB-порты (`/dev/cu.*` на macOS, `ttyUSB*`/`ttyACM*` на Linux), открывает каждый
и оставляет те, что отвечают как PN532. Если откликнулось несколько ридеров —
спросит, какой использовать; если ровно один — молча возьмёт его.

Это важно, потому что macOS переименовывает порт при перетыкании платы
(`usbserial-110` → `usbserial-210`), и любой прописанный намертво порт отвалится.

Чтобы закрепить конкретное устройство и пропустить сканирование, задайте connstring вручную:

```bash
BAMBU_NFC_DEV=pn532_uart:/dev/cu.usbserial-210 python3 bambu_nfc.py
```

### 3. Опционально: настройте libnfc

Для `bambu_nfc.py` не требуется — нужно только штатным `nfc-list` / `nfc-mfclassic`,
у которых автоопределения нет. libnfc из Homebrew ищет конфиг **внутри Cellar**,
а не в `/opt/homebrew/etc`:

```bash
NFC_DIR="/opt/homebrew/Cellar/libnfc/1.8.0/etc/nfc"
mkdir -p "$NFC_DIR/devices.d"

# разрешить интрузивное сканирование
printf 'allow_intrusive_scan = true\n' > "$NFC_DIR/libnfc.conf"

# описание PN532 UART (замените порт на свой — см. `ls /dev/cu.usbserial-*`)
printf 'name = "PN532 UART"\nconnstring = "pn532_uart:/dev/cu.usbserial-210"\n' \
  > "$NFC_DIR/devices.d/pn532_uart.conf"
```

Проверьте связь с ридером (без метки):

```bash
nfc-list
```

Должно быть `NFC device: PN532 UART opened`.

### 4. Соберите C-хелпер

Низкоуровневые операции с меткой выполняет `nfc_helper` (libnfc напрямую —
`nfc-mfclassic` на «гуляющем» теге делает частичные записи).
Скрипт **собирает его автоматически** при первом запуске, но можно и вручную:

```bash
cc nfc_helper.c -I/opt/homebrew/include -L/opt/homebrew/lib -lnfc -o nfc_helper
```

---

## Запуск

```bash
python3 bambu_nfc.py
```

### Язык интерфейса

Русский / английский. Выбор языка (по умолчанию — по системной локали, иначе английский):

```bash
python3 bambu_nfc.py --lang ru      # аргумент
BAMBU_LANG=ru python3 bambu_nfc.py  # переменная окружения
```

Скрипт по шагам:

1. Просит приложить метку, читает **UID**.
2. **Диагностика состояния** — определяет, засекречена ли метка:

   | Состояние | Засекречена |
   |---|---|
   | Оригинальный Bambu (data read-only, access `87878769`) | **ДА** |
   | Bambu, уже разлочена | НЕТ |
   | Перезаписана в NDEF (публичные ключи) | НЕТ |
   | Чистая / заводская (ключи `FFFFFFFFFFFF`) | НЕТ |
   | Неизвестная | ? |

3. Показывает **меню доступных действий** под текущее состояние:
   - Записать данные — URL, текст, Wi-Fi (SSID + пароль) или произвольные байты; при необходимости снимает защиту автоматически
   - Записать другую катушку — перенос данных филамента из сохранённого дампа `.mfd`
   - Собрать катушку Bambu — данные филамента из пресета или параметров
   - Показать содержимое метки (автоопределение URL / текст / Wi-Fi / raw)
   - Показать данные филамента Bambu (тип, цвет, вес, диаметр)
   - Снять дамп в файл
   - Стереть в заводское состояние

### Поддерживаемые типы данных

| Тип | Что записывается |
|---|---|
| URL | NDEF URI (`https://…`, `tel:`, `mailto:`, …) |
| Текст | NDEF Text (UTF-8) |
| Wi-Fi | NDEF Wi-Fi Simple Config (SSID + пароль, WPA2/AES) — подключение по касанию на поддерживаемых телефонах |
| Байты | Произвольный hex в data-блоки (без обёртки NDEF) |

Длинные значения автоматически раскладываются на несколько секторов.

### Клонирование и сборка катушки

- «Записать другую катушку» переносит данные филамента из сохранённого дампа `.mfd`
  (снятого через *Снять дамп в файл*) на текущую метку — с ключами под её UID и
  возвратом заводского доступа.
- «Собрать катушку Bambu» строит данные филамента из параметров — выберите пресет
  (в т.ч. пресеты, автоматически считанные из ваших дампов) или введите Material ID,
  тип, цвет, вес и температуры вручную.

⚠️ На Bambu-теге есть **RSA-2048 подпись**, сделанная приватным ключом Bambu, —
подделать её нельзя. У клона дампа подпись остаётся от *исходного* UID, у собранной
катушки её нет вовсе, поэтому принтер, проверяющий подпись, может их отклонить.
Работает на прошивках, которые подпись **не** проверяют (исторически — основной
случай); восстановление дампа на **свою же** метку работает полностью. Сами данные
филамента всегда пишутся корректно.

### Отладка

Подробный вывод по каждому блоку при записи:

```bash
BAMBU_DBG=1 python3 bambu_nfc.py
```

---

## Пример работы

Запись ссылки на **засекреченную оригинальную** метку Bambu (защита снимается автоматически):

```text
$ python3 bambu_nfc.py --lang ru
Bambu NFC — инструмент метки катушки
PN532 + libnfc. Диагностика состояния и доступные действия.

━━━ Определение метки ━━━
» Приложите тег к ридеру и нажмите Enter...
  ✓ UID: 02158BEF
  Диагностика секторов...
  Чтение содержимого...

━━━ Состояние метки ━━━
  Тип метки  : Оригинальный Bambu — ЗАСЕКРЕЧЕНА (data read-only)
  Засекречена: ДА
  Access-биты: 87878769
  data-блоки защищены от записи; для записи нужно снять защиту (сделаю автоматически).

━━━ Доступные действия ━━━
  1) Записать данные (URL / текст / Wi-Fi / raw)
  2) Записать другую катушку (из дампа)
  3) Собрать катушку Bambu (из параметров)
  4) Показать данные филамента Bambu
  5) Снять дамп в файл
  6) Стереть в заводское состояние
  0) Выход
? Выберите действие [1]: 1

━━━ Тип данных ━━━
  1) URL / ссылка
  2) Текст
  3) Сеть Wi-Fi (SSID + пароль)
  4) Произвольные байты (hex)
? Выберите действие [1]: 1
? Введите URL [https://bambulab.com]:
  ✓ Образ собран: URL: https://bambulab.com (секторов под данные: 1)
? Записать? Это необратимо (y/n) [y]: y
  Метка засекречена — снимаю защиту...
  unlock sector  0 OK
    … (секторы 1–15)
  Пишу образ (держите тег неподвижно)...
  write sector  0 (3 блоков)
  write sector  1 (4 блоков)
    … (секторы 2–15)
  ✓ Записано и проверено на уровне блоков
  Контрольное чтение...
  read sector  0 (4/4)
    … (секторы 1–15)

✓ Готово! На метке записано: URL: https://bambulab.com
```

Для **уже перезаписанной** метки диагностика покажет `Тип метки: Перезаписана в NDEF-метку`,
`Засекречена: НЕТ`, а в меню появится пункт **«Показать содержимое метки»**.

---

## Файлы

| Файл | Назначение |
|---|---|
| `bambu_nfc.py` | Главный интерактивный скрипт (меню + прогресс, RU/EN) |
| `nfc_helper.c` | C-хелпер libnfc: `uid` / `diag` / `unlock` / `write` / `read` |
| `bambu_keys.py` | Деривация KeyA/KeyB из UID; генерация keyfile |
| `build_target.py` | Сборка NDEF-образа (отдельно от `bambu_nfc.py`) |

Остальные `*.c` (`keytest`, `mapB`, `phase1_unlock`, `robust_write`, …) —
исследовательские утилиты, использованные при разборе формата; для работы не нужны.

### Отдельная генерация ключей

```bash
python3 bambu_keys.py <UID_hex> [keyfile.mfd]
# пример:
python3 bambu_keys.py 02158BEF fullkeys.mfd
```

---

## Важно знать

- **Перезапись необратима для назначения Bambu**: после записи URL принтер
  перестанет распознавать катушку. Метка при этом остаётся восстановимой
  (публичные/заводские ключи известны), её можно стереть или перезаписать снова.
- **iPhone не читает MIFARE Classic** (Core NFC не поддерживает этот чип) —
  записанный на такую метку URL на iPhone не откроется. Читают часть Android
  (с NXP-контроллером) и, конечно, сам PN532. Для универсальной ссылки-метки,
  читаемой любым телефоном, нужен чип **NTAG213/215**, а не Bambu-тег.

---

## Устранение неполадок

| Симптом | Решение |
|---|---|
| `No NFC device found` | Переподключите ридер и запустите заново — порт определяется автоматически. Задать принудительно: `BAMBU_NFC_DEV=pn532_uart:/dev/cu.usbserial-XXX`. Проверить порт: `BAMBU_NFC_DEV=... ./nfc_helper dev` |
| UID скачет / считается неверно | Метка «гуляет» — прижмите её к центру антенны и держите неподвижно |
| `write-fail` на секторе | Реальный отказ доступа — приложите метку плотнее и повторите |
| Скрипт не собирает `nfc_helper` | Соберите вручную (см. шаг 4), проверьте `xcode-select --install` |

---

## Поддержать

Если это сэкономило вам время — можно угостить меня кофе ☕

<div align="center">

[![Buy Me a Coffee](https://img.shields.io/badge/Buy%20Me%20a%20Coffee-buymeacoffee.com%2Fdisaldr-FFDD00?style=for-the-badge&logo=buymeacoffee&logoColor=black)](https://buymeacoffee.com/disaldr)

</div>

---

## Лицензия

Распространяется под [лицензией MIT](LICENSE).

---

## Дисклеймер

Инструмент создан для исследовательских и совместимых целей (использование
сторонних катушек, восстановление меток). Используйте на своих метках и
на свой риск.
